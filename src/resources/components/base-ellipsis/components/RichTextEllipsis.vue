<!-- 用于渲染富文本内容的子组件 -->
<template>
    <div ref="root" class="rich-text-ellipsis" @click="handleClick">
        <div v-if="!exceeded">
            <div v-html="content"></div>
        </div>
        <div v-if="exceeded && !expanded">
            <div v-html="ellipsis?.leading"></div>
            <span v-if="expandText" class="rich-text-ellipsis__text" @click.stop="clickHandle(1)">
                {{ expandText }}
            </span>
            <div v-html="ellipsis?.tailing"></div>
        </div>
        <div v-if="exceeded && expanded">
            <div v-html="content"></div>
            <span v-if="collapseText" class="rich-text-ellipsis__text" @click.stop="clickHandle(2)">
                {{ collapseText }}
            </span>
        </div>
    </div>
...
import type { EllipsisDirection } from './types';
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const createContainer = () => {
            ...
            const lineHeight = pxToNumber(
                originStyle.lineHeight === 'normal' ? props.lineHeight : originStyle.lineHeight
            );
        ...
        const calcEllipse = () => {
            if (container.offsetHeight <= maxHeight) {
                ...
                const ellipsised =
                    props.direction === 'middle' ? tailorMiddle([0, middle], [middle, end]) : tailor(0, end);
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        const tailor: (left: number, right: number) => EllipsisedValue = (left: number, right: number) => {
            ...
            if (props.direction === 'end') {
                container.innerHTML = props.content.slice(0, middle) + props.symbol + actionText;
            } else {
                container.innerHTML = actionText + props.symbol + props.content.slice(middle, end);
            }
